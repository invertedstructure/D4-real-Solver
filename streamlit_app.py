...
APP_TZ = timezone.utc  # UTC timestamps for auditability

st.set_page_config(page_title="OTC 4D Sanity Runner (v3.3)", layout="wide")
st.title("Odd-Tetra Certificate — 4D Sanity Runner (v3.3)")
st.caption("Shape-validated + CSV export + Runbook with timestamped logs, Export-all, and runbook.json download.")

# --- Session state for runbook + collected CSVs ---
if "runbook" not in st.session_state:
    st.session_state.runbook = []  # list of dict entries
if "csv_blobs" not in st.session_state:
    st.session_state.csv_blobs = {}  # name -> bytes

def now_iso():
    dt = datetime.now(APP_TZ)
    return dt.strftime("%Y-%m-%d %H:%M:%S"), dt.year

def add_runbook_entry(kind, note, csv_name=None, csv_bytes=None):
    ts, year = now_iso()
    entry = {"timestamp": ts, "year": year, "kind": kind, "note": note}
    st.session_state.runbook.append(entry)
    if csv_name and csv_bytes is not None:
        st.session_state.csv_blobs[csv_name] = csv_bytes
...
with tab1:
    ...
    if st.button("Run unit checks", type="primary"):
        try:
            ...
            if shapesj:
                enforce_chain_shapes(CXj, shapesj)
                enforce_chain_shapes(CYj, shapesj)
                enforce_map_shapes(Cmapj, shapesj, side="X")
                enforce_rep_lengths(repsd, shapesj)
            ...
            # CSV
            dfu = pd.DataFrame([unit_result]); csv_bytes = dfu.to_csv(index=False).encode("utf-8")
            st.download_button("Download unit-checks.csv", csv_bytes, "unit-checks.csv", "text/csv")
            add_runbook_entry("unit", "Unit checks executed", "unit-checks.csv", csv_bytes)
        except Exception as e:
            st.error(f"Validation or run error: {e}")
            add_runbook_entry("unit", f"ERROR: {e}")
    ...
    if sub == "Overlap (pairwise)":
        ...
        if st.button("Run overlap test"):
            try:
                ...
                if shapesj:
                    ...
                    enforce_map_shapes(C1j, shapesj, side="X")
                    enforce_map_shapes(C2j, shapesj, side="X")
                ...
                rows = [{"degree": k, "eq": v["eq"]} for k, v in res.items()]
                dfo = pd.DataFrame(rows); csv_bytes = dfo.to_csv(index=False).encode("utf-8")
                st.download_button("Download overlap-results.csv", csv_bytes, "overlap-results.csv", "text/csv")
                add_runbook_entry("overlap", "Overlap test executed", "overlap-results.csv", csv_bytes)
            except Exception as e:
                st.error(f"Validation or run error: {e}")
                add_runbook_entry("overlap", f"ERROR: {e}")
    else:
        ...
        if st.button("Run triangle coherence test"):
            try:
                ...
                st.write("PASS" if ok else "FAIL"); st.json(res)
                rows = [{"degree": k, "eq": v["eq"]} for k, v in res.items()]
                dft = pd.DataFrame(rows); csv_bytes = dft.to_csv(index=False).encode("utf-8")
                st.download_button("Download triangle-results.csv", csv_bytes, "triangle-results.csv", "text/csv")
                add_runbook_entry("triangle", "Triangle test executed", "triangle-results.csv", csv_bytes)
            except Exception as e:
                st.error(f"Validation or run error: {e}")
                add_runbook_entry("triangle", f"ERROR: {e}")
...
with tab3:
    ...
    if st.button("Run tower"):
        try:
            ...
            st.download_button("Download tower-hashes.csv", csv_bytes, "tower-hashes.csv", "text/csv")
            add_runbook_entry("tower", f"Tower run depth={len(seq)}", "tower-hashes.csv", csv_bytes)
            if novelty_step > 0:
                ...
                csv_bytes_n = dfn.to_csv(index=False).encode("utf-8")
                st.download_button("Download tower-novelty-hashes.csv", csv_bytes_n, "tower-novelty-hashes.csv", "text/csv")
                add_runbook_entry("tower-novelty", f"Novelty at step {novelty_step}", "tower-novelty-hashes.csv", csv_bytes_n)
            ...
            st.error(f"Validation or run error: {e}")
            add_runbook_entry("tower", f"ERROR: {e}")

with tab4:
    st.header("Runbook")
    st.markdown("A timestamped log of all actions in this session. Timestamps are ISO in UTC; `year` is included for filtering.")
    if len(st.session_state.runbook) == 0:
        st.info("No entries yet. Run a test in the other tabs.")
    else:
        df = pd.DataFrame(st.session_state.runbook)
        st.dataframe(df, use_container_width=True)
        # Export all CSVs as a single ZIP
        if st.session_state.csv_blobs:
            import zipfile
            zip_bytes = BytesIO()
            with zipfile.ZipFile(zip_bytes, "w", zipfile.ZIP_DEFLATED) as z:
                for name, data in st.session_state.csv_blobs.items():
                    z.writestr(name, data)
            st.download_button("Export all CSVs as zip", zip_bytes.getvalue(), "otc-results-bundle.zip", "application/zip")
        # NEW: Download runbook.json
        runbook_bytes = json.dumps(st.session_state.runbook, indent=2).encode("utf-8")
        st.download_button("Download runbook.json", runbook_bytes, "runbook.json", "application/json")

with tab5:
    st.markdown("""
### Notes
- Runbook entries are stored **in-session** and can be downloaded as `runbook.json` for local archival.
- All timestamps are in **UTC** with format `YYYY-MM-DD HH:MM:SS`.
- Use the 'Export all' ZIP to bundle all CSVs generated this session.
""")
