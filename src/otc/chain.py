...
import numpy as np
from .gf2 import to_bool, matmul_gf2, eq_gf2
...
def check_boundary_compat(CX, CY, Cmap, zlift=False, dX_signed=None, dY_signed=None, C_signed=None):
    """Check chain map law degreewise:
        d_Y(k) @ C_k  ==  C_{k-1} @ d_X(k)
    Only checks degrees where both C_k and C_{k-1} are provided to avoid shape errors at the bottom degree.
    """
    ok = True; details = []
    if not zlift:
        ...
        for k in degs:
            if (k-1) not in Cmap:
                # Skip check at bottom if C_{k-1} not provided
                continue
            Ck = to_bool(Cmap[k])
            Ckm1 = to_bool(Cmap[k-1])
            dY = CY.d(k); dX = CX.d(k)
            lhs = matmul_gf2(dY, Ck)      # (dim_{k-1} x dim_k) @ (dim_k x dim_k)
            rhs = matmul_gf2(Ckm1, dX)    # (dim_{k-1} x dim_{k-1}) @ (dim_{k-1} x dim_k)
            eq = eq_gf2(lhs, rhs)
...
def check_transport_homology(CX, CY, Cmap, c_dom, c_cod, k):
    from .gf2 import to_bool, in_image
    v_map = (to_bool(Cmap[k]) @ to_bool(c_dom).reshape(-1,1)) % 2
    v_cod = to_bool(c_cod).reshape(-1,1)
    diff = (v_map.astype(int) ^ v_cod.astype(int)) % 2
    dYkp1 = CY.d(k+1)
    in_im = in_image(dYkp1, diff)
    return in_im, v_map, v_cod, diff

def pairing_value(v_hi, v_lo, B=None, zlift=False, B_signed=None):
    from .gf2 import to_bool
    if not zlift:
        v_hi = to_bool(v_hi).reshape(-1,1)
        v_lo = to_bool(v_lo).reshape(-1,1)
        if B is not None:
            B = to_bool(B)
            return int((v_hi.T @ (B.astype(int)) @ v_lo) % 2)
        n = min(v_hi.shape[0], v_lo.shape[0])
        return int((v_hi[:n].T @ v_lo[:n]) % 2)
    else:
        v_hi = np.array(v_hi, dtype=int).reshape(-1,1)
        v_lo = np.array(v_lo, dtype=int).reshape(-1,1)
        if B_signed is not None:
            return int((v_hi.T @ B_signed @ v_lo).item())
        n = min(v_hi.shape[0], v_lo.shape[0])
        return int((v_hi[:n].T @ v_lo[:n]).item())
